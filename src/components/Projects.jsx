...
export default function Projects() {
  const projects = [
    {
      ...
      links: [
        ...
        { label: "Backend", url: "https://github.com/ekomh170/hris-backend-laravel-sib" }
      ]
    },
    {
      id: "02",
  ...
  const getBadgeStyles = (category) => {
    switch (category) {
      case 'frontend':
        return 'bg-[#1A1510] text-[#E89B3C]'; // Sunlit gold tint
      case 'automation':
        return 'bg-[#1A1018] text-[#C48AB8]'; // Sunset pink tint
      case 'backend':
      case 'database':
        return 'bg-[#152847] text-[#8BA3C8]'; // Sky blue tint
      default:
        return 'bg-[#111F3A] text-[#8BA3C8]'; // Ocean blue tint
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 max-w-5xl mx-auto">
          {projects.map((project) => (
            ...
            >
              ...
              <div className="border-t border-zen-border pt-5 mt-auto flex items-center gap-6 flex-wrap">
                {project.links ? (
                  project.links.map((repo, idx) => (
                    <a
                      key={idx}
                      href={repo.url}
                      target="_blank"
                      ...
                      aria-label={`View ${repo.label} repository of ${project.name} on GitHub`}
                    >
                      <svg
